...
local function StopNoFallDamage()
    ...
    Log("No Fall Damage выключён")
end

-- Переживает респавн - персонаж пересоздаётся, форсфилд ставится заново.
...
local function MoveToTargetSmart(targetPart, targetObj, kind)
    ...
    -- Ждём пока персонаж физически осядет (перестанет падать/скользить) -
    -- сразу после телепорта PathfindingService иногда не может построить
    -- путь от ещё "нестабильной" точки, отсюда и зависание на месте.
    do
        local character = LocalPlayer.Character
        local humanoid = character and character:FindFirstChildOfClass("Humanoid")
    ...
    if not moved and reason ~= "target_broken" then
        -- Первая попытка не удалась (возможно pathfinding не успел
        -- сориентироваться сразу после телепорта) - даём ещё немного
        -- осесть и пробуем ещё раз, прежде чем сдаваться.
        Log("Путь от вейпоинта 2 не построился с первого раза, пробую ещё раз")
        task.wait(0.5)
...
local OrangeWhiteTheme = {
    TextColor = Color3.fromRGB(70, 40, 15),
    Background = Color3.fromRGB(255, 248, 240),
    Topbar = Color3.fromRGB(255, 140, 30),
    Shadow = Color3.fromRGB(230, 150, 90),
    NotificationBackground = Color3.fromRGB(255, 255, 255),
    NotificationActionsBackground = Color3.fromRGB(255, 220, 180),
    TabBackground = Color3.fromRGB(255, 225, 195),
    TabStroke = Color3.fromRGB(255, 165, 90),
    TabBackgroundSelected = Color3.fromRGB(255, 140, 30),
    TabTextColor = Color3.fromRGB(120, 70, 20),
    SelectedTabTextColor = Color3.fromRGB(255, 255, 255),
    ElementBackground = Color3.fromRGB(255, 240, 225),
    ElementBackgroundHover = Color3.fromRGB(255, 225, 195),
    SecondaryElementBackground = Color3.fromRGB(255, 245, 235),
    ElementStroke = Color3.fromRGB(255, 180, 110),
    SecondaryElementStroke = Color3.fromRGB(255, 200, 150),
    SliderBackground = Color3.fromRGB(255, 170, 80),
    SliderProgress = Color3.fromRGB(255, 120, 0),
    SliderStroke = Color3.fromRGB(255, 200, 130),
    ToggleBackground = Color3.fromRGB(255, 230, 205),
    ToggleEnabled = Color3.fromRGB(255, 120, 0),
    ToggleDisabled = Color3.fromRGB(200, 190, 180),
    ToggleEnabledStroke = Color3.fromRGB(255, 150, 60),
    ToggleDisabledStroke = Color3.fromRGB(180, 170, 160),
    ToggleEnabledOuterStroke = Color3.fromRGB(255, 120, 0),
    ToggleDisabledOuterStroke = Color3.fromRGB(150, 140, 130),
    DropdownSelected = Color3.fromRGB(255, 225, 195),
    DropdownUnselected = Color3.fromRGB(255, 245, 235),
    InputBackground = Color3.fromRGB(255, 245, 235),
    InputStroke = Color3.fromRGB(255, 180, 110),
    PlaceholderColor = Color3.fromRGB(200, 160, 130),
}

local Window = Rayfield:CreateWindow({
    Name = "Тереяки с Клаудом",
    LoadingTitle = "Тереяки с Клаудом",
    LoadingSubtitle = "",
...
pcall(function() Window:SetTheme(OrangeWhiteTheme) end)
...
local function applySkipSelection(selected)
    ...
    Log("Skip List обновлён: " .. table.concat(names, ", "))
end
...
Tabs.Main:CreateToggle({
    ...
    Callback = function(value)
        ...
        if value then
            ...
            Rayfield:Notify({ Title = "Тереяки с Клаудом", Content = "Запущено", Duration = 2 })
        else
            ClearPathVisuals()
            SomeFlag2 = false
            StatusText = "Ожидание"
            Log("Автоферма ВЫКЛЮЧЕНА")
            Rayfield:Notify({ Title = "Тереяки с Клаудом", Content = "Остановлено", Duration = 2 })
        end
    end
})
        ...
        if value == "Lockpick" then
            task.spawn(function()
                if CountTools("Lockpick") > 0 then
                    ...
                    Rayfield:Notify({ Title = "Break Method", Content = "Нет лок-пиков, иду к дилеру", Duration = 2 })
                    BuyLockpickBatch()
                end
            end)
        end
...
local statusPara = Tabs.Stats:CreateParagraph({ Title = "Статус", Content = "Загрузка..." })
local safesPara = Tabs.Stats:CreateParagraph({ Title = "Сейфы", Content = "0/0" })
local registersPara = Tabs.Stats:CreateParagraph({ Title = "Кассы", Content = "0/0" })
local remainingPara = Tabs.Stats:CreateParagraph({ Title = "Осталось", Content = "0/0" })
local suggestionPara = Tabs.Stats:CreateParagraph({ Title = "Совет", Content = "Загрузка..." })
...
task.spawn(function()
    while true do
        if Settings.Enabled then
            setParagraph(statusPara, "Статус", StatusText)
            setParagraph(safesPara, "Сейфы", AvailableSafesCount .. "/" .. TotalSafesCount)
            setParagraph(registersPara, "Кассы", AvailableRegistersCount .. "/" .. TotalRegistersCount)
            setParagraph(remainingPara, "Осталось", (AvailableSafesCount + AvailableRegistersCount) .. "/" .. (TotalSafesCount + TotalRegistersCount))
            setParagraph(suggestionPara, "Совет", SuggestionText)
        else
            setParagraph(statusPara, "Статус", "Ожидание")
            setParagraph(safesPara, "Сейфы", "0/0")
            setParagraph(registersPara, "Кассы", "0/0")
            setParagraph(remainingPara, "Осталось", "0/0")
            setParagraph(suggestionPara, "Совет", "Запусти ферму")
        end
        pcall(readStatsGui)
        pcall(readCashAmountValue)
...
Rayfield:Notify({ Title = "Тереяки с Клаудом", Content = "Загружено", Duration = 2 })
