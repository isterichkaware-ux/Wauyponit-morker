...
local function HackSafeWithLockpick(safeObj)
    ...
    local function getFreshLockpick()
        local lp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Lockpick")
        if not lp then
            lp = LocalPlayer.Backpack and LocalPlayer.Backpack:FindFirstChild("Lockpick")
            if lp then
                EquipTool("Lockpick")
                task.wait(0.5)
                lp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Lockpick")
            end
        end
        if not lp then return nil, nil, nil end
        local usesVal = lp:FindFirstChild("Uses")
        if usesVal and usesVal.MaxValue and usesVal.Value >= usesVal.MaxValue then
            lp = LocalPlayer.Backpack and LocalPlayer.Backpack:FindFirstChild("Lockpick")
            if lp then
                EquipTool("Lockpick")
                task.wait(0.5)
                lp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Lockpick")
                if lp then usesVal = lp:FindFirstChild("Uses") end
            end
        end
        return lp, lp and lp:FindFirstChild("Uses"), lp and lp:FindFirstChild("Remote")
    end
    ...
    while Settings.Enabled and safeObj and safeObj.Parent do
        ...
        local lp, usesVal, remote = getFreshLockpick()
        if not lp or not remote then
            if not BuyLockpickBatch() then break end
            EquipTool("Lockpick")
            task.wait(1)
            lp, usesVal, remote = getFreshLockpick()
        ...
        until lockpickGui or waited >= 1.5
