...
local function findCrowbarDealer()
    ...
    for _, shop in ipairs(shopz:GetChildren()) do
        ...
        local crowbarStock = stocks and stocks:FindFirstChild("Crowbar")
        if crowbarStock and crowbarStock.Value and crowbarStock.Value > 0 then
...
local function BuyCrowbar()
    ...
    local moved = MoveToTarget(dealerPart)
...
local function DropLockpick(tool)
    ...
    local character = LocalPlayer.Character
    local hrp = character and character:FindFirstChild("HumanoidRootPart")
    if not tool or not dropRemote or not hrp then return false end
    return pcall(dropRemote.FireServer, dropRemote, tool, nil, hrp.Position)
end

-- Простой, как в оригинальном Criminality_Farm: без наведения камеры, без
-- поиска/масштабирования GUI, без счётчика попыток - чистый слепой
-- InvokeServer("S"...)/InvokeServer("D"...). Если лок-пик не смог даже
-- начать (S не вернул токен) - выбрасываем именно этот инстанс (DropLockpick),
-- и следующая попытка естественным образом возьмёт другой лок-пик из
-- рюкзака (findToolByName всегда берёт первый оставшийся).
local function tryLockpickTargetViaRemote(target)
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    local character = LocalPlayer.Character
    local tool = (character and character:FindFirstChild("Lockpick"))
        or (backpack and backpack:FindFirstChild("Lockpick"))
...
local function HackSafeWithLockpick(safeObj)
    ...
    local HACK_TIME_BUDGET = 30

    while Settings.Enabled and safeObj and safeObj.Parent and tick() - startTime < HACK_TIME_BUDGET do
        ...
        local opened, reason = tryLockpickTargetViaRemote(safeObj)
        Log("Попытка лок-пика: " .. tostring(reason))
    ...
    if not ok then
        Log("Сейф так и не открылся лок-пиком за " .. HACK_TIME_BUDGET .. " сек, меняю таргет")
    end
