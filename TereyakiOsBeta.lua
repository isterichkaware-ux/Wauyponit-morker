...
-- Список конкретных объектов (точные имена), которые никогда не трогаем
-- (не ходим к ним, не считаем их доступными целями вообще).
local SkipList = {
...
local function isTargetSkipped(targetName)
    return SkipList[tostring(targetName)] == true
end
...
local function MoveToTargetSmart(targetPart, targetObj, kind)
    ...
    -- Если персонаж уже физически внутри этой буферной зоны (близко к
    -- вейпоинту 2) - не повторяем весь путь через вейпоинт 1 и телепорт,
    -- просто идём напрямую к цели обычным умным путём. Радиус свой у каждой
    -- зоны (жёстко небольшой, чтобы не ловить ложные срабатывания в других
    -- частях карты, случайно оказавшихся в старом общем радиусе 60).
    local character = LocalPlayer.Character
    local hrp = character and character:FindFirstChild("HumanoidRootPart")
    local zoneRadius = zone.radius or 10

    if hrp and (hrp.Position - zone.waypoint2).Magnitude <= zoneRadius then
        Log("Уже внутри зоны " .. zone.name .. " (радиус " .. zoneRadius .. "), иду к " .. tostring(targetObj and targetObj.Name) .. " напрямую")
    ...
    if not moveSuccess then
        ...
        if RetryCount >= 3 then
            ...
            if hrp then
                ...
                RetryCount = 0
                moveSuccess = MoveToTarget(mainPart, nil, "dealer")
                if not moveSuccess then
...
local function FindLockpickDealer()
    ...
    if closestDealer then
        Log("Найден дилер с лок-пиком, расстояние: " .. math.floor(closestDist))
    else
        Log("Дилер с лок-пиком не найден")
    end
    return closestDealer
end
...
-- GUI минигры лок-пика в текущей версии игры визуально выглядит иначе, чем в
-- старых скриптах (кружок с надписью "LOCKPICK"), и мы не знаем точных имён
-- его внутренних Frame/Label. Поэтому ищем generic: любой GuiObject в
-- PlayerGui, чьё имя содержит "lockpick" (без учёта регистра), поднимаемся
-- до ближайшего контейнера прямо под ScreenGui и вешаем на него UIScale.
local LOCKPICK_GUI_SCALE = 3

local function findLockpickGuiRoot()
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
    if not playerGui then return nil end

    for _, obj in ipairs(playerGui:GetDescendants()) do
        if obj:IsA("GuiObject") and tostring(obj.Name):lower():find("lockpick") then
            local node = obj
...
local function scaleLockpickZones(root)
    root = root or findLockpickGuiRoot()
    ...
    uiScale.Scale = LOCKPICK_GUI_SCALE
    return true
end

-- Постоянный наблюдатель: как только что-то с "lockpick" в имени появляется
-- в PlayerGui (от любого сейфа, в любой момент) - сразу увеличиваем.
local lockpickGuiWatcherStarted = false
local function ensureLockpickGuiWatcher()
    if lockpickGuiWatcherStarted then return end
    lockpickGuiWatcherStarted = true

    local playerGui = LocalPlayer:WaitForChild("PlayerGui")
    playerGui.DescendantAdded:Connect(function(child)
        if child:IsA("GuiObject") and tostring(child.Name):lower():find("lockpick") then
            task.wait(0.05)
            pcall(scaleLockpickZones)
        end
    end)

    pcall(scaleLockpickZones)
end
ensureLockpickGuiWatcher()

-- Кликаем по сейфу (через VirtualInputManager, по экранной точке), чтобы
-- открыть минигру. Возвращает true, если клик вообще удалось сделать
-- (не гарантирует что GUI откроется - это проверяется отдельно).
-- Просто проверяем/ждём, что LockpickGUI реально появился (и, значит, уже
-- увеличен постоянным watcher-ом сверху) - никаких кликов сюда не шлём,
-- сам взлом идёт через InvokeServer ниже. Это только для того, чтобы
-- скрипт "всегда видел" GUI перед тем, как дёргать ремоут.
local function waitForLockpickGui(timeoutSeconds)
    timeoutSeconds = timeoutSeconds or 2

    local existing = findLockpickGuiRoot()
    if existing then
        scaleLockpickZones(existing)
        return existing
    end
    ...
    while waited < timeoutSeconds do
        ...
        local gui = findLockpickGuiRoot()
        if gui then
            scaleLockpickZones(gui)
            return gui
        end
        task.wait(0.05)
        waited = waited + 0.05
...
local function getLockpickToolState()
    local character = LocalPlayer.Character
    local backpack = LocalPlayer:FindFirstChild("Backpack")

    local tool = character and character:FindFirstChild("Lockpick")
    if not tool then
        local inBackpack = backpack and backpack:FindFirstChild("Lockpick")
        if inBackpack then
            EquipTool("Lockpick")
            task.wait(0.5)
            tool = character and character:FindFirstChild("Lockpick")
        end
    end
    if not tool then return nil, nil end

    local uses = tool:FindFirstChild("Uses")
    if uses and uses.MaxValue and uses.Value >= uses.MaxValue then
        -- Этот лок-пик исчерпан, ищем свежий в рюкзаке
        local fresh = backpack and backpack:FindFirstChild("Lockpick")
        if fresh then
            EquipTool("Lockpick")
            task.wait(0.5)
            tool = character and character:FindFirstChild("Lockpick")
            uses = tool and tool:FindFirstChild("Uses")
        end
    end
...
-- Кнопка старта минигры лок-пика в этой игре рисуется/кликабельна только
-- когда камера (или палец на мобиле) реально наведена на сейф - это не
-- ProximityPrompt с удержанием клавиши, а именно клик по экранной точке.
-- Поэтому перед кликом принудительно наводим камеру на сейф, чтобы
-- WorldToScreenPoint гарантированно попадал "на экран" независимо от того,
-- куда игрок смотрел до этого.
local function aimCameraAtPart(part)
    ...
    local character = LocalPlayer.Character
    local hrp = character and character:FindFirstChild("HumanoidRootPart")
    local eyePos = hrp and (hrp.Position + Vector3.new(0, 2, 0)) or camera.CFrame.Position

    camera.CameraType = Enum.CameraType.Scriptable
    camera.CFrame = CFrame.new(eyePos, part.Position)

    return previousCFrame, previousType
end

local function restoreCamera(previousCFrame, previousType)
    local camera = Workspace.CurrentCamera
...
-- Жёсткая фиксация камеры на весь период взлома (не только на момент клика):
-- игрок иногда случайно уводит камеру во время взлома пальцем/мышкой, из-за
-- чего минигра теряется. Держим камеру принудительно наведённой на сейф
-- каждый кадр, пока идёт попытка, независимо от ввода игрока.
local LockpickCameraLockConnection = nil
...
local function startLockpickCameraLock(part)
    ...
    if not LockpickCameraLockConnection then
        LockpickCameraPreviousType = camera.CameraType
    end
    camera.CameraType = Enum.CameraType.Scriptable

    if LockpickCameraLockConnection then
        LockpickCameraLockConnection:Disconnect()
    end

    LockpickCameraLockConnection = RunService.RenderStepped:Connect(function()
        ...
        -- Нормальный третьеличный отступ позади персонажа (как обычная
        -- камера игры), а не первое лицо "в глазах" - иначе камеру
        -- невозможно отдалить, пока замок активен.
...
local function stopLockpickCameraLock()
    if LockpickCameraLockConnection then
        LockpickCameraLockConnection:Disconnect()
        LockpickCameraLockConnection = nil
    end
    local camera = Workspace.CurrentCamera
...
local function clickToOpenLockpickMinigame(safeObj)
    local aimPart = safeObj:FindFirstChild("PosPart")
        or safeObj:FindFirstChild("MainPart")
        or safeObj.PrimaryPart
...
-- Сам взлом: как раньше, через InvokeServer("S"...)/InvokeServer("D"...) на
-- Remote самого тула лок-пика. GUI при этом уже увеличен watcher-ом выше -
-- если он реально появился, ждём его, только потом дёргаем ремоут.
local function tryLockpickTargetViaRemote(target, tool)
    local remote = tool and tool:FindFirstChild("Remote")
    if not remote or not remote:IsA("RemoteFunction") then
        return false, "lockpick_remote_missing"
    end

    -- Камера уже наведена на цель постоянным замком (startLockpickCameraLock,
    -- запущен в HackSafeWithLockpick) - этого достаточно, отдельный клик по
    -- экрану не нужен. Сама минигра открывается через InvokeServer("S"...).
    task.wait(0.2)

    local startOk, token = pcall(function()
        return remote:InvokeServer("S", target, "s")
    end)

    if not startOk or type(token) ~= "number" then
        return false, "lockpick_start_failed"
    end

    task.wait(0.25)

    local finishOk = pcall(function()
        return remote:InvokeServer("D", target, "s", token)
    end)
...
local function HackSafeWithLockpick(safeObj)
    ...
    -- Жёсткий лимит попыток в ОДНОМ потоке, без отдельной корутины-таймера
    -- (та была ненадёжной - гонка между потоками, счётчик иногда не успевал
    -- взвестись раньше, чем цикл уходил в очередной InvokeServer). Просто
    -- считаем реальные попытки взлома и обрываем после MAX_ATTEMPTS, что бы
    -- ни происходило.
    local MAX_ATTEMPTS = 5
    local attemptCount = 0

    local function pickUntriedLockpick()
        local character = LocalPlayer.Character
        local backpack = LocalPlayer:FindFirstChild("Backpack")

        local equipped = character and character:FindFirstChild("Lockpick")
        if equipped then
            local uses = equipped:FindFirstChild("Uses")
            local exhausted = uses and uses.MaxValue and uses.Value >= uses.MaxValue
            if not exhausted and not triedTools[equipped] then
                return equipped, uses
            end
        end

        if backpack then
            for _, item in ipairs(backpack:GetChildren()) do
                if item.Name == "Lockpick" and not triedTools[item] then
                    local uses = item:FindFirstChild("Uses")
                    local exhausted = uses and uses.MaxValue and uses.Value >= uses.MaxValue
                    if not exhausted then
                        EquipSpecificTool(item)
                        local freshTool = character and character:FindFirstChild("Lockpick")
                        return freshTool, freshTool and freshTool:FindFirstChild("Uses")
                    end
                end
            end
        end
    ...
    while Settings.Enabled and safeObj and safeObj.Parent do
        ...
        if attemptCount >= MAX_ATTEMPTS then
            Log("Достигнут лимит попыток (" .. MAX_ATTEMPTS .. "), меняю таргет")
            break
        end

        StatusText = "Взлом лок-пиком: попытка " .. (attemptCount + 1) .. "/" .. MAX_ATTEMPTS

        if CountTools("Lockpick") == 0 then
            Log("Лок-пики закончились, докупаю")
            if not BuyLockpickBatch() then
                stopLockpickCameraLock()
                StatusText = "Ожидание"
                return false, "lockpick_buy_failed"
            end
            triedTools = {}
            if safeObj.Parent and not (broken and broken.Value) then
                MoveToTargetSmart(safeObj:FindFirstChild("MainPart") or safeObj.PrimaryPart, safeObj, "target")
            end
        end

        local tool = pickUntriedLockpick()
        if not tool then
            Log("Все лок-пики в рюкзаке уже пробовали и не сработали, докупаю новые")
            if not BuyLockpickBatch() then
                stopLockpickCameraLock()
                StatusText = "Ожидание"
                return false, "lockpick_buy_failed"
        ...
        local opened, reason = tryLockpickTargetViaRemote(safeObj, tool)
        Log("Попытка лок-пика " .. attemptCount .. "/" .. MAX_ATTEMPTS .. " (" .. tool:GetDebugId() .. "): " .. tostring(reason))

        if opened then
            local completedAt = tick()
            while safeObj.Parent
                and not (values:FindFirstChild("Broken") and values.Broken.Value)
                and tick() - completedAt < 2
            do
                task.wait(0.10)
            end
            break
    ...
    if not ok then
        Log("Сейф так и не открылся лок-пиком за " .. attemptCount .. " попыток, меняю таргет")
    end
...
local function performDepositRequest(events, cash)
    ...
    if not MoveToTarget(atmMainPart, nil, "atm") then
        return false
    end
...
local function MainFarmLoop()
    ...
    while true do
        ...
        if Settings.AutoAllowance and tick() - StatsInfo.LastAllowanceClaimAttempt > 30 then
        ...
        if moveSuccess then
            local hackSuccess
            local isRegister = nextTarget.Name:lower():find("register") ~= nil

            if isRegister then
                hackSuccess = HackWithFists(nextTarget)
            elseif Settings.BreakMethod == "Lockpick" then
                local lockpickReason
                hackSuccess, lockpickReason = HackSafeWithLockpick(nextTarget)
                if lockpickReason == "target_unavailable" then
                    Log("Цель недоступна (уже открыта кем-то), пропускаю навсегда")
                    Settings.ProcessedList[nextTarget] = true
                    continue
                end
            else
                ...
                Log("Открываю сейф")
                hackSuccess = HackSafe(nextTarget)
            end
            ...
            if hackSuccess then
                ...
                local waitTime = getPostBreakWaitSeconds(nextTarget.Name)
                Log("Стою на месте " .. waitTime .. " сек после взлома (" .. nextTarget.Name .. ")")
                task.wait(waitTime)
            else
